...
class AuthController extends Controller
{
    ...
    {
        $phone = preg_replace('/\+/', '', $request->input('phone'));
        ...
        $validator = Validator::make($request->all(), [
            ...
            'phone'     => [
                            'required', 'digits_between:5,15', 'regex:/^[0-9]+$/',
                            Rule::unique('users')->where(function ($query) use ($request) {
                                return $query->where('user_type', 'customer');
                            }),
                        ],
        ...
        if($validator->fails()){
            ...
            if ($errors->has('email')) {
                return response()->json(['status' => false, 'message' => $errors->first('email'), 'user_id' => null  ], 200);
            }
        
            if ($errors->has('phone')) {
                return response()->json(['status' => false, 'message' => $errors->first('phone'), 'user_id' => null  ], 200);
            }

            if ($errors->has('name')) {
                return response()->json(['status' => false, 'message' => $errors->first('name'), 'user_id' => null  ], 200);
            }

            if ($errors->has('password')) {
                return response()->json(['status' => false, 'message' => $errors->first('password'), 'user_id' => null  ], 200);
            }
            return response()->json(['status' => false, 'message' => 'Something went wrong', 'user_id' => null  ], 200);
        }
