...
class AdminController extends Controller
{
    ...
    public function admin_dashboard(Request $request)
    {
        ...
        if (Auth::user()->user_type == 'staff' && Auth::user()->shop_id != null){
            ...
            $a['salesAmount'] = Order::where('order_success', 1)->where('delivery_status', 'delivered')->where('payment_status','paid')->sum('grand_total');
        ...
        if ($request->has('category_id') && $request->category_id !== '0') {
            ...
            $product->whereHas('category', function ($q) use ($childIds) {
        ...
        $orderMonthGraph = Cache::remember('orderMonthGraph', 86400, function () use ($days) {
            ...
            $monthOrders = Order::where('order_success', 1)->whereMonth('created_at', Carbon::now()->month)->whereYear('created_at', Carbon::now()->year)
                ->get()
        ...
        $salesYearGraph = Cache::remember('salesYearGraph', 86400, function () {
            ...
            $data = Order::select(\DB::raw('MONTH(created_at) as month, COUNT(*) as count, SUM(grand_total) as total'))
                ->where('order_success', 1)->where('delivery_status', 'delivered')->where('payment_status', 'paid')
                ->where('created_at', '>=', Carbon::now()->subMonths(11))
                ->groupBy('month')
                ->get();
        ...
        if(!empty($request->all())){
            Session::put('top_report_filter', $request->all());
        }else{
            Session::forget('top_report_filter');
        }
        return view(
            'backend.dashboard',
            compact('searches', 'counts', 'topProducts', 'orderMonthGraph', 'days','shopOrderYearGraph', 'orderYearGraph', 'salesYearGraph', 'salesMonthGraph','year','category_id')
        );
    }
    
    public function exportTopSellingReport(){
        $data = [];
        if(Session::has('top_report_filter')){
            $data = Session::get('top_report_filter');
        }
       
        $category_id = (isset($data['category_id']) && $data['category_id'] != '') ? $data['category_id'] : null;
        
        return Excel::download(new TopsellingExport($category_id), 'top_selling_report_' . now()->format('Y-m-d_H-i-s') . '.xlsx');
    }
