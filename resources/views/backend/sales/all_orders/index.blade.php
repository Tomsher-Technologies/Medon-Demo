...
<div class="card">
    <form class="" action="" id="sort_orders" method="GET">
        <div class="card-header row gutters-5">
            ...
            <!--</div>-->
            <div class="col-sm-12">
                <h5 class="mb-md-2 h6">All Orders</h5>
            </div>
    ...
    <!--                    <a class="dropdown-item" href="#" data-toggle="modal" data-target="#exampleModal">
            ...
            <div class="col-sm-12 row">
                ...
                <div class="col-sm-4 mt-2">
                        <select class="form-control aiz-selectpicker" name="payment_gateway_status" id="payment_gateway_status">
                            <option value="">{{translate('Filter by Order Status')}}</option>
                            <option value="2" @if ($payment_gateway_status == '2') selected @endif>{{translate('Failed')}}</option>
                            <option value="1" @if ($payment_gateway_status == '1') selected @endif>{{translate('Success')}}</option>
                        </select>
                    </div>
                <div class="col-sm-2 mt-2">
                    <div class="form-group mb-0">
                        <button type="submit" class="btn btn-info">Filter</button>
                        <a class="btn btn-warning" href="{{ route('all_orders.index') }}" >Clear</a>
                    </div>
                </div>
            </div>
        </div>
                ...
                <tbody id="order-table">
                    ...
                    <tr style="background:{{$statusColor}}">
                        ...
                        @else
                            ...
                            @else
                                <td class="myInputGroupSelect text-center"></td>
                            @endif
                        @endif
                        ...
                        <td class="text-center">
                            ...
                            <a class="btn btn-soft-info btn-icon btn-circle btn-sm @if ($payment_gateway_status == '2') d-none @endif" href="{{ route('invoice.download', $order->id) }}" title="Download Invoice">
                                <i class="las la-download"></i>
                            </a>
                            @if ($payment_gateway_status == '2') 
                                <button type="button" class="btn btn-soft-success btn-icon btn-circle btn-sm move-success" data-order="{{$order->id}}" title="Move To Success List" style="font-size: large;font-weight:700;">
...
//        }
        ...
        $(document).on('click','.move-success',function(){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response == 1){
                                swal("Successfully moved to order success list!", {
                                    icon: "success",
                                });
                                setTimeout(function() {
                                    window.location.reload();
